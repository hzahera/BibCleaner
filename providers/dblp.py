...
_STOP_WORDS = frozenset(
    ...
    "its their it as we our be has have been using towards toward "
...
class DblpProvider(Provider):
    ...
    def lookup(self, query: ProviderQuery) -> ProviderResult:
        ...
        published_data = normalize(published) if published else None
        preprint_authors = normalize(preprint).get("authors", []) if preprint else []

        return ProviderResult(
            published_data=published_data,
            preprint_authors=preprint_authors,
            matched=bool(published or preprint),
        )
